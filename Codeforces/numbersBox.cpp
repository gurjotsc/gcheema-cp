...
ll solve() {
    ...
    for(auto& i : grid) {
        ...
        for(auto& j : i) {
            cntNeg += (j < 0);
            mnabs = min(mnabs, abs(j));
            sm += abs(j);
        }
    }
    
    //mnabs*2 because abs(mnabs) is included in sm so the negative 
    //would be removing the included one and then adding the negative
    return (!(cntNeg%2)) ? sm : sm-(mnabs*2); 
}
