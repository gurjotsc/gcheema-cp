...
string solve() {
    ll n, k; cin >> n >> k;
    vector<ll> a(n); cin >> a;
    if(k == 1) return "Yes"; //bubble sort

    vector<vector<ll>> b(k);

    for(int i = 0; i < n; ++i) b[i%k].push_back(a[i]); //each vector stores swappable values

    //reverse sort each vector
    for(int i = 0; i < k; ++i) sort(b[i].rbegin(), b[i].rend());
    
    //sort a to comapre it with
    sort(all(a));

    //lets say there are two numbers that can be swapped but they appear in the same b[i]
    //then these numbers can never be in the right location cuz they have to be in one another's spot
    for(int i = 0; i < n; ++i) {
        if(b[i%k].back() != a[i]) return "No";
        b[i%k].pop_back();
    }
