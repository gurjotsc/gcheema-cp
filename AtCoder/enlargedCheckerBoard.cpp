...
void solve() {
    ll n, a, b; cin >> n >> a >> b;
    vector<vector<bool>> gridColors (n, vector<bool> (n, false));
    for(int i = 0; i < n; ++i) {
        if(i > 0) gridColors[i][0] = !gridColors[i-1][0];
        for(int j = 1; j < n; ++j) gridColors[i][j] = !gridColors[i][j-1];
    }
    for(int i = 0; i < n; ++i) {
        for(int x = 0; x < a; ++x) {
            for(int j = 0; j < n; ++j) {
                bool color = gridColors[i][j];
                for(int y = 0; y < b; ++y) cout << (color) ? '#' : ',';
            }
            cout << "\n";
        }    
    }
